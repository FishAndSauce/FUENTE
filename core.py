from transformations import generate_load_duration_curve
import pandas as pd
import os
...
generators_included_characteristics_dataframe['gas_price_cost'] = np.where(generators_included_characteristics_dataframe['Fuel Type'] == 'Gas', gas_price_per_mwh, 0)
generators_included_characteristics_dataframe['coal_price_cost'] = np.where(generators_included_characteristics_dataframe['Fuel Type'] == 'Coal', coal_price_per_mwh, 0)
generators_included_characteristics_dataframe['biomass_price_cost'] = np.where(generators_included_characteristics_dataframe['Fuel Type'] == 'Biomass', biomass_fuel_price_per_mwh, 0)
# calculate total fuel cost ($/MW/yr) accounting for thermal efficiency of plant
generators_included_characteristics_dataframe['total_fuel_cost'] = 365 * 24 * (generators_included_characteristics_dataframe['gas_price_cost'] + generators_included_characteristics_dataframe['coal_price_cost'] + generators_included_characteristics_dataframe['biomass_price_cost']) / generators_included_characteristics_dataframe['Thermal Efficiency']
# calculate total emmissions cost (KgCO2e/MW/yr)
generators_included_characteristics_dataframe['total_emissions_cost'] = 24 * 365 * generators_included_characteristics_dataframe['Carbon Emissions (KgCO2e/MWh)'] * carbon_price_per_kg


# calculate total variable cost
generators_included_characteristics_dataframe['total_variable_cost'] = generators_included_characteristics_dataframe['VOM/year ($/MW/yr)'] + generators_included_characteristics_dataframe['total_emissions_cost'] + generators_included_characteristics_dataframe['total_fuel_cost']
# calculate total fixed cost
generators_included_characteristics_dataframe['total_fixed_cost'] = generators_included_characteristics_dataframe['Annualised Capital ($/MW/yr)'] + generators_included_characteristics_dataframe['FOM ($/MW/yr)']

generator_cost_curve_dict = dict()
for generator in generators_included_list:
    generator_cost_curve_dict[generator] = {'gradient': generators_included_characteristics_dataframe.loc[generator, 'total_variable_cost'], 'y-intercept': generators_included_characteristics_dataframe.loc[generator, 'total_fixed_cost']}

print generator_cost_curve_dict

# caclulate lowest envelope
# find_lowest_cost_envelope(generator_cost_curve_dict)
